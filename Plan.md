...
**22. После успешной заявки "Купить", данные о платеже со статусом APPROVED сохраняются в базе данных PostgreSQL.**
  ...
  *Шаги:*
    ...
    8. Поле "Владелец" ввести: Ivanov Ivan
    9. В поле "CVC/CVV" ввести: 999
    10. Нажать кнопку "Продолжить"
	11. Дождаться сообщения "Успешно. Операция одобрена банком"
	12. Проверить, что в таблицаъ появидись данные об операции:
	  - Выполнить select-postgres-table.cmd
...
**23. После успешной заявки "Купить в кредит", данные о платеже со статусом APPROVED сохраняются в базе данных PostgreSQL.**
...
24. После отклоненной заявки "Купить", данные о заявке со статусом DECLINED сохраняются в базе данных PostgreSQL.
  ...
  *Шаги:*
    ...
    3. Нажать кнопку "Купить"
    4. Проверить, что название формы - "Оплата по карте"
    5. В поле "Номер карты" ввести: 4444 4444 4444 4442 
    6. В поле "Месяц" ввести: 08
    7. В поле "Год" ввести: 22
    8. Поле "Владелец" ввести: Ivanov Ivan
    9. В поле "CVC/CVV" ввести: 999
    10. Нажать кнопку "Продолжить"
	11. Дождаться сообщения "Ошибка. Банк отказал а проведении операции"
	12. Проверить, что в таблицаъ появидись данные об операции:
	  - Выполнить select-postgres-table.cmd
...
26. После успешной заявки "Купить", данные о платеже со статусом APPROVED сохраняются в базе данных MySQL.
  ...
  *Ожидаемый результат:* Таблица payment_entity содержит строку операции оплаты, в поле amount значение 4500000, в поле status значение APPROVED. Таблица order_entity содержит строку операции покупки. Поля transaction_id и payment_id содержат одинаковые значения.

27. После успешной заявки "Купить в кредит", данные о платеже со статусом APPROVED сохраняются в базе данных MySQL.
...
28. После отклоненной заявки "Купить", данные о заявке со статусом DECLINED сохраняются в базе данных MySQL.
  ...
  *Ожидаемый результат:* Таблица payment_entity содержит строку операции оплаты, в поле status значение DECLINED. Таблица order_entity содержит строку операции покупки. Значения полей transaction_id и payment_id содержат одинаковые значения.

29. После отклоненной заявки "Купить в кредит", данные о завке со статусом DECLINED сохраняются в базе данных MySQL.
...
*Ожидаемый результат:* Таблица payment_entity не содержит записей. Таблица credit_request_entity содержит строку операции заявки, в поле status значение DECLINED. Таблица order_entity содержит строку операции покупки. Значения полей payment_id и bank_id содержат одинаковые значения.
